...
function logout() {
  ...
  // Logging out returns to the app as a guest — it never forces the
  // login/signup screen. The user can sign in again any time they want.
  loadConversationsFromLocalStorage();
  initSidebar();
  updateAuthMenuUI();
  ...
  showToast('Logged out. You can keep chatting as a guest, or sign in again anytime.');
}
...
function initUserMenu() {
  ...
  $('loginMenuItem') && $('loginMenuItem').addEventListener('click', () => {
    userDropdown.classList.remove('open');
    openAuthScreen('login');
  });

  $('authCloseBtn') && $('authCloseBtn').addEventListener('click', closeAuthScreen);
  const authScreenEl = $('authScreen');
  if (authScreenEl) {
    authScreenEl.addEventListener('click', e => { if (e.target === authScreenEl) closeAuthScreen(); });
  }
}
...
window.onload = function() {
  // Home page/chat is always shown directly — login/signup is optional,
  // available any time from the user menu, never a forced gate.
...
/** Show the login/signup overlay on top of the app — called only when the
 *  user chooses to sign in/up, never automatically. */
...
/** Dismiss the login/signup overlay and return to the app as-is
 *  (still logged in, or still a guest — whichever it was). */
...
/** Toggle dropdown items and sidebar labels between guest and signed-in state. */
function updateAuthMenuUI() {
  ...
  if (loginItem)   loginItem.style.display   = signedIn ? 'none' : '';
  if (profileItem) profileItem.style.display = signedIn ? '' : 'none';
  if (logoutItem)  logoutItem.style.display  = signedIn ? '' : 'none';
  ...
  if (!signedIn) {
    ...
    const ddName  = $('userDdName');
    const ddEmail = $('userDdEmail');
    if (ddName)  ddName.innerText  = 'Guest';
    if (ddEmail) ddEmail.innerText = 'Not signed in';
  }
}
/* ═══════════════════════════════════════════════════════════════════════
